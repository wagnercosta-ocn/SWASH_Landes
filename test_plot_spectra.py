...
# USER INPUT
# -------------------------
data_folder = "/scratch/work/langercostaw/swash_landes/swash_cases/AUT"
...
def read_aut_file(file_path):
    ...
    while i < len(lines):
        ...
        #    frequencies.append(growth*frequencies[i-1])
...
# LOAD ALL FILES
# -------------------------
files = sorted(glob.glob(os.path.join(data_folder, "AUT*")))
...
for (tlat, tlon) in target_points:
    ...
    # Colorbar
    cbar = plt.colorbar(pcm, ax=ax, pad=0.1,shrink=0.85)
    cbar.set_label("Spectral Density (m2 Hz-1)")
    ...
    plt.tight_layout()
    output_dir="/scratch/work/langercostaw/codes_new/outputs/rose_plots/"
    ...
    np.save(os.path.join(output_dir,"mean_spectrum_"+str(tlon)+"_"+str(tlat)+"_exp4.npy"),mean_spectrum)
